...
import type { ModelVariant } from "@/lib/backgroundRemoval"
...
export type BrushMode = "off" | "erase" | "restore"
...
export interface CompressToolState {
  preset: string
  quality: number
  stripExif: boolean
}

export interface ConvertToolState {
  format: string
  profile: string
  bg: string
}
...
export interface BgRemoveToolState {
  model: ModelVariant
  bg: string
  format: string
  brushMode: BrushMode
...
export interface ToolState {
  crop: CropToolState
  compress: CompressToolState
  convert: ConvertToolState
  rotate: RotateToolState
  bgremove: BgRemoveToolState
  watermark: WatermarkToolState
}

export type ToolId = keyof ToolState
