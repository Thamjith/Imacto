...
export interface BgBrush {
  ready: boolean
  canUndo: boolean
  canRedo: boolean
  editedUrl: string | null
  version: number
  initBaseline: (cutoutUrl: string, image: LoadedImage) => Promise<void>
  paintSegment: (segment: BrushSegment) => void
  endStroke: () => void
  undo: () => void
  redo: () => void
  reset: () => void
 ...
 */
export function useBgBrush(): BgBrush {
  const workCanvasRef = useRef<HTMLCanvasElement | null>(null)
  const originalRef = useRef<HTMLImageElement | null>(null)
  const undoStackRef = useRef<ImageData[]>([])
  const redoStackRef = useRef<ImageData[]>([])
  const lastCutoutRef = useRef<string | null>(null)
  ...
  const initBaseline = useCallback(
    async (cutoutUrl: string, image: LoadedImage) => {
  ...
  const paintSegment = useCallback(({ x0, y0, x1, y1, mode, radius }: BrushSegment) => {
    const work = workCanvasRef.current
    const ctx = work?.getContext("2d")
    if (!work || !ctx) return
  ...
  const undo = useCallback(() => {
    ...
    redoStackRef.current.push(stack.pop() as ImageData)
