...
import {
  MODEL_OPTIONS,
  MODEL_REPO_URL,
  fetchLatestModelVersion,
  modelSize,
  type ModelVariant,
} from "@/lib/backgroundRemoval"
import { type BgRemoveToolState, type BrushMode } from "@/constants/tools"
...
interface ModelChooserProps {
  ...
  onChoose: (id: ModelVariant) => void
  disabled?: boolean
}

function ModelChooser({ selected, onChoose, disabled }: ModelChooserProps) {
  return (
...
const BRUSH_MODES: ChipOption[] = [
  { id: "off", label: "Off" },
  { id: "erase", label: "Erase" },
  { id: "restore", label: "Restore" },
]

type Phase = "idle" | "downloading" | "checking"

interface BgRemovePanelProps {
  state: BgRemoveToolState
  set: (patch: Partial<BgRemoveToolState>) => void
}

export function BgRemovePanel({ state, set }: BgRemovePanelProps) {
  ...
  const download = async (version: string | null, variant: ModelVariant) => {
    setError(null)
  ...
  const switchVariant = (variant: ModelVariant) => {
    set({ model: variant })
      ...
      <PanelSection label="manual refine">
        {bgBrush?.ready ? (
          <ChipGroup
            ...
            onChange={(brushMode) => set({ brushMode: brushMode as BrushMode })}
          />
        ) : (
